...
func create_db() {
    ...
    myDB.org = "influxdata"
    myDB.token = "nJgOAKW4A1ghivS1vHynHp29HcN7Tj8y" // from your influxdb UI
    myDB.url = "http://10.102.46.51:80"//from your influxdb info
...
func encode_action_format1(plmn string, cellid string) clientmodel.ActionDefinition {
	...
	format1=[]int64{0,1,1,0,0,1,0,176,82,82,85,46,80,114,98,84,111,116,85,108,1,32,0,0,0,176,82,82,85,46,80,114,98,84,111,116,68,108,1,32,0,0,64,3,231}
	//for e2test
...
func encode_action_format3() clientmodel.ActionDefinition {
    ...
    format3=[]int64{0,1,3,64,0,0,0,208,67,65,82,82,46,87,66,67,81,73,68,105,115,116,0,0,64,64,1,0,64,3,231}
	//variable part is not presetnt in action def format 3
...
func encode_action_format1_new() clientmodel.ActionDefinition {
	var format1 []int64
...
func encode_actionsToBeSetup(meid string) clientmodel.ActionsToBeSetup {
	...
	*/
	var tmp_new int64 = tmp_act_id
	var lol_new *int64 = &tmp_new
	s_new := clientmodel.ActionToBeSetup{
		ActionID:         lol_new,
		ActionType:       &actionType,
		ActionDefinition: encode_action_format1_new(),
		SubsequentAction:  &clientmodel.SubsequentAction{
			SubsequentActionType: &subsequentActionType,
			TimeToWait:           &timeToWait,
		},
...
func (c *Control) handleIndication(params *xapp.RMRParams) (err error) {
	...
	indicationMsg, err := e2ap.GetIndicationMessage(params.Payload)
	/*if err != nil {
		fmt.Printf("Error fetching indication message: %v\n", err)
	} else {
		fmt.Printf("Indication Message: %+v\n", indicationMsg)
	}*/
	if err != nil {
		xapp.Logger.Error("Failed to decode RIC Indication message: %v", err)
		return
...
func (c *Control) writeCellMetrics_db(cellMetrics *map[string]interface{}) {
	...
	// Extract and convert RRU_PrbTotDl and RRU_PrbTotUl
	var rruPrbTotDl, rruPrbTotUl float64
	var okDl, okUl bool

	// Log the raw data to check
	xapp.Logger.Info("Received cellMetrics:", *cellMetrics)

	// Handle RRU_PrbTotDl
	switch v := (*cellMetrics)["RRU_PrbTotDl"].(type) {
	case float64:
		rruPrbTotDl = v
		okDl = true
	case int:
		rruPrbTotDl = float64(v)
		okDl = true
	case int32:
		rruPrbTotDl = float64(v)
		okDl = true
	...
	case string:
		var err error
		rruPrbTotDl, err = strconv.ParseFloat(v, 64)
		if err == nil {
			okDl = true
		} else {
			xapp.Logger.Error("RRU_PrbTotDl parsing error:", err)
		}
	default:
		xapp.Logger.Error("RRU_PrbTotDl type mismatch, actual type:", reflect.TypeOf(v))
	}

	// Handle RRU_PrbTotUl
	switch v := (*cellMetrics)["RRU_PrbTotUl"].(type) {
	case float64:
		rruPrbTotUl = v
		okUl = true
	case int:
		rruPrbTotUl = float64(v)
		okUl = true
	case int32:
		rruPrbTotUl = float64(v)
		okUl = true
	...
	case string:
		var err error
		rruPrbTotUl, err = strconv.ParseFloat(v, 64)
		if err == nil {
			okUl = true
		} else {
			xapp.Logger.Error("RRU_PrbTotUl parsing error:", err)
		}
	default:
		xapp.Logger.Error("RRU_PrbTotUl type mismatch, actual type:", reflect.TypeOf(v))
	}

	// Log error and return if parsing fails
	if !okDl || !okUl {
		xapp.Logger.Error("RRU_PrbTotDl or RRU_PrbTotUl invalid!")
		return
	}

	// Create and write point
	p := influxdb2.NewPointWithMeasurement("cellMetrics").
		AddField("RRU_PrbTotDl", rruPrbTotDl).
		AddField("RRU_PrbTotUl", rruPrbTotUl).
		// SetTime(time.Now())
		SetTime(time.Now().In(time.FixedZone("Asia/Taipei", 8*3600)))

	if err := writeAPI.WritePoint(context.Background(), p); err != nil {
		xapp.Logger.Error("Failed to write Cell Metrics to InfluxDB:", err)
	} else {
		xapp.Logger.Info("Successfully wrote Cell Metrics to InfluxDB")
	}
}
